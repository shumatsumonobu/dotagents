...
├── CHANGELOG.md                         # 全コンポーネント共通の変更履歴
├── CONTRIBUTING.md                      # このファイル
├── LICENSE
└── README.md
```

各プラグインは自己完結した配布単位。変更履歴はリポジトリ直下の `CHANGELOG.md` に集約する。

バージョン番号は `plugin.json` にも `marketplace.json` にも書かない。書かない場合、Claude Code は
解決したコミット SHA をバージョンとして扱うため、コミットすれば利用者に更新が届く。手で番号を
上げる作業は不要。
...
1. 対象コンポーネントの該当ファイル（エージェント/スキル/ルール等）を修正
2. リポジトリ直下の `CHANGELOG.md` を更新（見出しに対象コンポーネント名を書く）
3. 対象の README.md を更新（ユーザー影響がある場合）
4. ローカルテスト（下の「ローカルでの実行方法」）

### ローカルでの実行方法

**プラグイン** — マーケットプレイス経由のインストールは不要。ソース修正後に Claude Code を
再起動すれば反映:
